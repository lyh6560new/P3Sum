...
summarization_name_mapping = {
    ...
    "/home2/yhliu/controlpoli/ssd-lm/cnn_dm_500_raw": ("article", "highlights"),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/cnn_dm_500_raw": ("article", "highlights"),
    "/home2/yhliu/simplex-diffusion-main/biased_datasets/center": ("article", "highlights"),
    "/home2/yhliu/simplex-diffusion-main/biased_datasets/left": ("article", "highlights"),
    "/home2/yhliu/simplex-diffusion-main/biased_datasets/right": ("article", "highlights"),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/politics_biased/left":('text','gold'),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/politics_biased/center":('text','gold'),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/politics_biased/right":('text','gold'),  
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/politics_500_raw":('text','gold'), 
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/xsum_500_raw":("document", "summary"),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/cnn_dm_biased/left":("article", "highlights"),
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/cnn_dm_biased/center":("article", "highlights"),  
    "/home2/yhliu/controlpoli/ssd-lm/Sampled_Datasets/cnn_dm_biased/right":("article", "highlights"),
    ...
    "Sampled_Datasets/cnn_dm_500_raw": ("article", "highlights"),
    "Sampled_Datasets/xsum_500_raw":("document", "summary"),
    "Sampled_Datasets/politics_500_raw":('text','label'),
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args, diffusion_args,ctr_args = parser.parse_json_file(
    ...
    #check if control
    if not diffusion_args.if_control:
        ctr_args = None
    # Setup logging
    logging.basicConfig(
    ...
    if model_args.model_name_or_path:
        ...
        # if extend_window:
        #     #load a normal model and save the weights of position embeddings to weights
        #     config_normal = RobertaDiffusionConfig.from_pretrained(
        #     model_args.model_name_or_path,
        #     self_condition=diffusion_args.self_condition,
        #     self_condition_zeros_after_softmax=diffusion_args.self_condition_zeros_after_softmax,
        #     deepmind_conditional=diffusion_args.deepmind_conditional,
        ...
        #     use_auth_token=True if model_args.use_auth_token else None,
        # )
        #     model_normal = RobertaForDiffusionLM.from_pretrained(
        #     model_args.model_name_or_path,
        #     from_tf=bool(".ckpt" in model_args.model_name_or_path),
        ...
        #     )
        #     #copy first 512 position embeddings to next 200 embeddings
        #     weights = model_normal.roberta.embeddings.position_embeddings.weight.cpu().detach().numpy()
        #     shape = weights.shape
        #     #rebuild position embeddings
        #     model.roberta.embeddings.position_embeddings = torch.nn.Embedding(shape[0]*2, shape[1])
        #     extended_weights = np.concatenate([weights, weights], axis=0)
        #     with torch.no_grad():
        #         model.roberta.embeddings.position_embeddings.weight.copy_(torch.tensor(extended_weights))
    else:
        logger.info("Training new model from scratch")
        model = RobertaForDiffusionLM.from_config(config)

    # if extend_window:
    #     #copy first 200 position embeddings to next 200 embeddings
    #     weights = model.roberta.embeddings.position_embeddings.weight.cpu().detach().numpy()
    #     shape = weights.shape
    #     #rebuild position embeddings
    #     model.roberta.embeddings.position_embeddings = torch.nn.Embedding(shape[0]*2, shape[1])
    #     extended_weights = np.concatenate([weights, weights], axis=0)
    ...
    if training_args.do_predict:
        ...
        if not diffusion_args.if_control:
            ...
            prefix = "test_"+ctr_args.ctr_model_name.split('/')[-1] +"_lr_" + str(ctr_args.decode_ctr_lr)    + "_idx_" + str(ctr_args.ctr_opt_label_idx)
            metrics = trainer.evaluate(test_dataset, metric_key_prefix=prefix)
        max_predict_samples = (
